...
## A squash commit concatenating pre-review messages was a repository default, not an inherent property — and it took three attempts to stop proposing a remembered-step fix for it

**What happened:** PR #55 opened as two `docs/LESSONS.md` entries; by merge time, review had cut one entirely. The resulting squash-merge commit, `main` @ `6b50621`, put the false "two entries" claim — including a **superseded instruction**, not just a stale label — into three sites. Re-derived directly from the merged artefact, not trusted from a prior description of it: `git log -1 --format='%B' 6b50621`, 105 lines.

```
$ git log -1 --format='%B' 6b50621 | grep -n 'two LESSONS.md entries'
1:docs: two LESSONS.md entries from PR #47's re-review (checker vs. grep, test-count basis) (#55)
3:* docs: two LESSONS.md entries from the PR #47 re-review (checker vs. grep, test-count basis)

$ git log -1 --format='%B' 6b50621 | grep -n 'Second entry\|State the merge-base'
12:Second entry: a test count is measured against a base commit, not against
...
**The fix:** set `squash_merge_commit_message=PR_BODY` and `squash_merge_commit_title=PR_TITLE` at the repository level (`gh api -X PATCH repos/<owner>/<repo> -f squash_merge_commit_message=PR_BODY -f squash_merge_commit_title=PR_TITLE`), confirmed by a **separate GET**, not the PATCH response, independently by two people. This removes the mechanism that put a squashed commit's frozen message into history at all — structurally, not by anyone remembering to pass a flag. **It is not a complete close, and should not be recorded as one:** what's frozen shrinks from four time-separated commit messages nobody re-reads to one field — the PR title and body — that stay live and visible on the PR page through the whole review, editable up to the instant of merge. That is a much smaller, far more visible staleness surface, not its elimination — a near-miss inside this very PR proved it live: this entry's own prior draft, recommending the now-superseded two-flag fix, was still sitting in PR #56's body after the setting had already made it moot, and would have shipped as a second superseded-instruction-in-permanent-history event by the identical mechanism (a field authored before the ruling that invalidated it, unread at merge) had it not been caught before merging.
